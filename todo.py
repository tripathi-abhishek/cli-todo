...
$ ./todo report           # Statistics"""
	sys.stdout.buffer.write(todohelp.encode(
		'utf8'))		# Print the Usage in UTF-8 Encoding as the default print() generates unexpected results.


def addToList(st):

	# Function to add a New Todo in the todo.txt file.
	# If exists todo.txt then appends the task in the first line.
...
def showList():
	...
	if os.path.isfile('todo.txt'):
	    ...
	    for line in data:
	    	st += '[{}] {}'.format(ct, line)
	    	ct -= 1
	    # Print the Tasks in Reverse Order in UTF-8 Encoding as the default print() generates unexpected results.
...
def markDone(num):
	...
	if os.path.isfile('todo.txt'):
	    ...
	    if num > ct or num <= 0:
	    	...
	    	with open("todo.txt", 'w') as todoFileMod:
	    		# Produces output according to the availability of done.txt file.
	    		if os.path.isfile('done.txt'):
			    	...
			    	with open("done.txt", 'w') as doneFileMod:
			    		for line in data:
			    			if ct == num:
			    				doneFileMod.write(
			    					"x "+datetime.today().strftime('%Y-%m-%d')+" "+line)
			    			else:
			    				todoFileMod.write(line)
			    			ct -= 1
